"""
Dedicated scraper for ElDiario.es articles.
"""
import os
...
class ElDiarioScraper:
    ...
    def __init__(self, config: Dict):
        ...
        self.output_dir = os.path.join('output', 'competitors', 'eldiario')
    ...
    def get_article_data(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Extract authors from meta tags (common in eldiario.es)
            authors = []
    ...
    def get_recent_articles(self, days_back: int = 1, max_articles: int = 10) -> List[Dict]:
        """Get recent articles from ElDiario.es.
...
def get_articles(config: Dict, days_back: int = 1, max_articles: int = 10) -> List[Dict]:
    ...
    Returns:
        List of article dictionaries
    """
    scraper = ElDiarioScraper(config)
